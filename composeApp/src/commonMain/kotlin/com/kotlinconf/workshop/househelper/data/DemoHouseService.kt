...
class DemoHouseService : HouseService {
    ...
    private val areas = MutableStateFlow(demoAreas)

    private val devices = MutableStateFlow(demoDevices)

    override fun getAreas(): Flow<List<Area>> = areas

    override fun getDevicesForArea(areaId: String): Flow<List<Device>> = devices.map { deviceList ->
    ...
    override suspend fun toggleFavorite(deviceId: DeviceId): Boolean {
        ...
        when (device) {
            is LightDevice -> updateDevice(device.copy(isFavorite = !device.isFavorite))
            is SwitchDevice -> updateDevice(device.copy(isFavorite = !device.isFavorite))
            is HumidityDevice -> updateDevice(device.copy(isFavorite = !device.isFavorite))
            is ThermostatDevice -> updateDevice(device.copy(isFavorite = !device.isFavorite))
            is CameraDevice -> updateDevice(device.copy(isFavorite = !device.isFavorite))
        }
        return true
    }

    override fun getFavoriteDevices(): Flow<List<Device>> = devices.map { deviceList ->
        deviceList.filter { device -> device.isFavorite }
    }
}
