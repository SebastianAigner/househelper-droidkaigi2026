...
fun AreasContent(
    ...
    val favoritesExpanded by favoritesViewModel.expanded.collectAsStateWithLifecycle()
    ...
    LazyColumn(
        ...
        if (favoriteDevices.isNotEmpty()) {
            item {
                RoomSection(
                    ...
                    onExpand = { isExpanded ->
                        favoritesViewModel.expand(isExpanded)
                    },
                    onClick = { device -> favoritesViewModel.onDeviceClicked(device) },
                    onLongClick = { device ->
                        when (device) {
        ...
        // Show all areas
        items(areas) { area ->
            val areaViewModel: AreaViewModel = koinViewModel(key = area.id) {
                parametersOf(area.id)
            }
            val devices by areaViewModel.devices.collectAsStateWithLifecycle()
            val expanded by areaViewModel.expanded.collectAsStateWithLifecycle()
            ...
            RoomSection(
                ...
                devices = devices,
                onExpand = { isExpanded ->
                    areaViewModel.expand(isExpanded)
                },
                onClick = { device -> areaViewModel.onDeviceClicked(device) },
                onLongClick = { device ->
                    when (device) {
                        is LightDevice -> onNavigateToLightDetails(device.deviceId)
                        is CameraDevice -> onNavigateToCameraDetails(device.deviceId)
